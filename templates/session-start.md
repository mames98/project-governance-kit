...
If a session begins substantive work without acknowledging the state board, [DECISION_OWNER] should redirect before proceeding.

*Template note — [DECISION_OWNER]: the person or role who gates decisions for this scope; in a single-operator project, that's you. Fill it everywhere it appears; delete this note after filling.*
...
2. **Update "Open decisions"** — add any newly-surfaced decisions (each naming its owner); close any that resolved during the session.
...
- A [DECISION_OWNER] decision is made or explicitly deferred
...
- Update the board whenever work state changes meaningfully, not just at convenient moments. "Meaningfully" means: a commit lands, a session starts or completes a work thread, a [DECISION_OWNER] decision is made or deferred, a blocking condition changes.
...
The failure mode is recoverable: if the board gets stale, it gets updated. The protection is human vigilance plus session-boundary discipline. The failure mode to prevent is leaving a stale board that looks authoritative to a fresh session.
...
- **[DECISION_OWNER]** may amend the board ad-hoc at any time for clarity or to surface new context. Such amendments are valid immediately and are incorporated at the next read.
...
**Multi-operator note:** with multiple humans on a project, designate one board owner ([BOARD_OWNER_ROLE] stays a single role), and each entry in the board's "Open decisions" section carries the owner who gates it. The separation logic above is unchanged — only the number of humans holding [DECISION_OWNER] values varies.
